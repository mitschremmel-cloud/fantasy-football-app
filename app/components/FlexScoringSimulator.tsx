...
export default function FlexScoringSimulator() {
  ...
  const fetchData = async () => {
    ...
    const res = await fetch('/api/nflverse/flex', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ year: year, weights: sanitizedWeights }),
    });
    if (!res.ok) {
      console.error("Flex API error:", await res.text());
      return;
    }
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-lg text-white">
      <h2 className="text-xl font-bold mb-4">Flex Scoring Simulator</h2>
      <p className="text-sm text-slate-400 mb-4">Top 60 nach den Top 20 RBs/WRs und Top 10 TEs.</p>
      ...
      <div className="overflow-x-auto">
        ...
        <thead>
          <tr className="text-slate-400 border-b border-slate-700 cursor-pointer">
            ...
            <th className="p-2" onClick={() => requestSort('position')}>Pos</th>
        ...
        <tbody>
          {sortedData.slice(0, 60).map((p, i) => (
            <tr key={i} className="border-b border-slate-700 hover:bg-slate-700/30">
              ...
              <td className="p-2 text-indigo-400">{(p.pointsPerGame ?? 0).toFixed(1)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="h-[400px] w-full mt-10 bg-slate-900 p-4 rounded">
        <h3 className="text-white mb-4">Punkte Vergleich (Total) (Top 60)</h3>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={sortedData.slice(0, 60).map((p, i) => ({
            rank: i + 1,
            name: p.name,
            standard: p.standardTotalPoints,
            simulated: p.totalPoints
          }))}>
            ...
            <Tooltip
              ...
              labelFormatter={(label, payload) => {
                const player = payload[0]?.payload?.name;
                return player ? `${player} (Rank ${label})` : `Rank ${label}`;
              }}
            />
            <Legend verticalAlign="top" align="right" wrapperStyle={{ top: 0, right: 0 }} />
            <Line type="monotone" dataKey="standard" stroke="#94a3b8" name="Standard (Total)" strokeWidth={2} dot={false} />
            <Line type="monotone" dataKey="simulated" stroke="#6366f1" name="Simuliert (Total)" strokeWidth={2} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>

      <div className="h-[400px] w-full mt-10 bg-slate-900 p-4 rounded">
        <h3 className="text-white mb-4">Punkte Vergleich (Pts/G) (Top 60)</h3>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={sortedData.slice(0, 60).map((p, i) => ({
            rank: i + 1,
            name: p.name,
            ...
            />
            <Legend verticalAlign="top" align="right" wrapperStyle={{ top: 0, right: 0 }} />
            <Line type="monotone" dataKey="standard" stroke="#94a3b8" name="Standard (Pts/G)" strokeWidth={2} dot={false} />
            <Line type="monotone" dataKey="simulated" stroke="#6366f1" name="Simuliert (Pts/G)" strokeWidth={2} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
