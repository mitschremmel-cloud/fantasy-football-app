...
export interface TEScoringWeights {
...
export async function calculateTESimulation(year: number | number[], weights: TEScoringWeights) {
  const years = Array.isArray(year) ? year : [year];
  const allResults: any[] = [];

  for (const y of years) {
    const data = await getStatsByPosition(y, 'Te');
  ...
  const playerMap = new Map<string, any>();
    ...
    const calc = (w: TEScoringWeights) => Object.keys(s).reduce((sum, k) => sum + s[k as keyof typeof s] * w[k as keyof typeof w], 0);
