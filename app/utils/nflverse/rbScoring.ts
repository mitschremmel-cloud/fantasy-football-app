...
export interface RBScoringWeights {
...
export async function calculateRBSimulation(year: number | number[], weights: RBScoringWeights) {
  const years = Array.isArray(year) ? year : [year];
  const allResults: any[] = [];

  for (const y of years) {
    const data = await getStatsByPosition(y, 'RB');
  ...
  const playerMap = new Map<string, any>();
    ...
    const calc = (w: RBScoringWeights) => Object.keys(s).reduce((sum, k) => sum + s[k as keyof typeof s] * w[k as keyof typeof w], 0);
